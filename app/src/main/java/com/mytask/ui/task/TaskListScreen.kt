        ...
        if (tasks.isEmpty()) {
            ...
            LazyColumn(
                ...
                items(
                    ...
                    val courseName = courses.find {
                        it.id == task.courseId
                    }?.name ?: "Mata Kuliah belum dipilih"
        ...
        Row(
            ...
            Column(modifier = Modifier.weight(1f)) {
                ...
                if (!task.isCompleted && deadlineInfo != null) {
                    ...
                    StatusLabel(
                        text = task.completedAt?.let { formatCompletedLabel(it) }
                            ?: "Tugas selesai"
                    )
                }
            }
...
private fun DeadlineChip(info: DeadlineInfo) {
    val backgroundColor = if (info.isOverdue) {
        MaterialTheme.colorScheme.errorContainer
    } else {
        MaterialTheme.colorScheme.secondaryContainer
    }

    val contentColor = if (info.isOverdue) {
...
private fun EmptyTaskList(paddingValues: PaddingValues) {
    Column(
        ...
        Text(
            text = "Tambahkan tugas pertama kamu untuk mulai mengatur aktivitas akademik.",
