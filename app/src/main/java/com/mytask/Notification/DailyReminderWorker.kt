...
class DailyReminderWorker(appContext: Context, workerParams: WorkerParameters) : CoroutineWorker(appContext, workerParams) {
    override suspend fun doWork(): Result {
        ...
        return try {
            ...
            if (activeTaskNotificationEnabled) checkActiveTasks(tasks) else NotificationHelper.cancelActiveTasksNotification(applicationContext)
    ...
    private suspend fun checkScheduleNotificationWindow(database: MyTaskDatabase, schedules: List<ScheduleEntity>, settingsRepository: SettingsRepository) {
        ...
        schedules.filter { it.dayOfWeek == today || it.dayOfWeek == ReminderScheduler.EVERY_DAY }.forEach { schedule ->
            ...
            val start = Calendar.getInstance().apply {
                if (schedule.dayOfWeek == ReminderScheduler.EVERY_DAY) {
            ...
            val course = schedule.courseId?.let { id -> courses.find { it.id == id } }
            ...
            NotificationHelper.showScheduleNotification(applicationContext, schedule.id.toString(), "🕒 Kuliah yang akan datang", message, countdownUntilMillis = startMillis)
            ReminderScheduler.cancelScheduleCountdown(applicationContext, schedule.id)
        }
    }
    ...
    private fun checkActiveTasks(tasks: List<TaskEntity>) {
        ...
        if (activeTasks.isEmpty()) { NotificationHelper.cancelActiveTasksNotification(applicationContext); return }
    ...
    private fun checkDeadlineTasks(tasks: List<TaskEntity>, reminderDays: Int) {
        ...
        val reminderStart = Calendar.getInstance().apply { time = today.time; add(Calendar.DAY_OF_YEAR, reminderDays) }
        tasks.forEach { task ->
            if (task.isCompleted || task.deadline == null) { NotificationHelper.cancelTaskNotification(applicationContext, task.id.toString()); return@forEach }
            val deadlineDay = Calendar.getInstance().apply { time = task.deadline!!; resetTime() }
            ...
            if (!shouldShow) { NotificationHelper.cancelTaskNotification(applicationContext, task.id.toString()); return@forEach }
            if (isOverdue) {
                val overdueDays = getOverdueDays(today, deadlineDay)
                NotificationHelper.showOverdueTaskNotification(applicationContext, task.id.toString(), task.deadline!!.time, "⚠️ ${task.title}", buildOverdueTaskDetail(task, overdueDays))
            } else NotificationHelper.showTaskNotification(applicationContext, task.id.toString(), "📝 ${task.title}", buildUpcomingTaskDetail(task))
        }
    }
    ...
    private fun getDeadlineText(now: Calendar, deadline: Date): String {
        ...
        return when { days < 0L -> "terlambat ${-days} hari"; days == 0L -> "hari ini"; days == 1L -> "1 hari lagi"; else -> "$days hari lagi" }
    }
    private fun getOverdueDays(today: Calendar, deadline: Calendar): Long = TimeUnit.MILLISECONDS.toDays(today.timeInMillis - deadline.timeInMillis).coerceAtLeast(1L)
    private fun getPriorityText(priority: Int): String = when (priority) { 1 -> "Tinggi"; 2 -> "Sedang"; 3 -> "Rendah"; else -> "Normal" }
    private fun Calendar.resetTime() { set(Calendar.HOUR_OF_DAY, 0); set(Calendar.MINUTE, 0); set(Calendar.SECOND, 0); set(Calendar.MILLISECOND, 0) }
}
